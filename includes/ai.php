...
- 스펙을 쉼표로 이어 붙이거나 데이터베이스 필드를 읽어 주듯 나열하지 마세요.
- 자연스러운 완전한 문장으로 작성하고, 문장마다 하나의 핵심 정보만 전달하세요.
- intro 첫 문장은 제품의 성격을 설명하고, 다음 문장은 핵심 사양이 실제 사용에서 무엇을 의미하는지 설명하세요.
- "주요 사양은 발표", "출시 상태는", "표시되어 있습니다", "출시." 같은 기계적인 표현과 문장 파편을 사용하지 마세요.
- 발표일과 출시일은 중요할 때만 자연스럽게 한 문장에 포함하고 반복하지 마세요.
- 스펙을 단순 나열하지 말고 구매 판단에 도움이 되도록 해석하되, FACTS 밖의 체감 성능은 단정하지 마세요.
- 벤치마크나 측정값이 없으면 "여유 있게 처리", "쾌적하다", "뛰어난 성능", "오래 사용할 수 있다" 같은 성능·품질 결론을 쓰지 마세요.
- 과장, 광고 문구, 최상급 표현, 키워드 반복을 피하세요.
- 널리 쓰이는 영문 기술명과 모델명은 유지하되, 어려운 약어는 "OIS(광학식 손떨림 보정)"처럼 처음 한 번만 괄호로 설명하세요.
- inches, screen-to-body ratio, wide, telephoto처럼 자연스럽게 옮길 수 있는 일반 용어는 각각 인치, 화면 대 본체 비율, 광각, 망원처럼 한국어로 작성하세요.
- cautions는 확인 가능한 제한 사항만 작성하고, 근거가 없으면 빈 배열로 반환하세요.
- JSON 이외의 텍스트나 마크다운을 출력하지 마세요.
JSON 구조:
{
  "intro": "서로 자연스럽게 이어지는 3~5문장 제품 개요",
  "verdict": "제품명과 스펙을 나열하지 않는 한 문장 핵심 판단",
